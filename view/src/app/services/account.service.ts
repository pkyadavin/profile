...
export class AccountService {
  ...
  constructor(
    ...
    this.currentUserSubject$ = new BehaviorSubject<ApplicationUser>(JSON.parse(localStorage.getItem('Masters-currentUser')));
  }
  ...
  login(model: ApplicationUserLogin) : Observable<ApplicationUser>  {
    return this.http.post(`${environment.webApi}/Account/login`, model).pipe(
      map((user : ApplicationUser) => {
        ...
        if (user) {
          localStorage.setItem('Masters-currentUser', JSON.stringify(user));
  ...
  logout() {
    localStorage.removeItem('Masters-currentUser');
